...
/// <summary>
/// Describes the physics shapes of a model or standalone vphys resource.
/// Wraps native <c>CPhysicsData</c> the same way <see cref="Model"/> wraps <c>IModel</c>.
/// </summary>
public sealed class PhysicsGroupDescription : Resource
{
	CPhysicsData _native;

	public override bool IsValid => _native.IsValid;
	...
	internal static PhysicsGroupDescription FromNative( CPhysicsData native )
	{
		...
		if ( NativeResourceCache.TryGetValue<PhysicsGroupDescription>( instanceId, out var existing ) )
		{
	...
	/// <summary>
	/// Enumerate every <see cref="Surface"/> in this physics description.
	/// </summary>
	...
	void Refresh()
	{
		...
		{
			var tagCount = _native.GetTagCount( attributeIndex );
			var tags = new List<StringToken>( tagCount );

			for ( int tagIndex = 0; tagIndex < tagCount; tagIndex++ )
			{
				tags.Add( new StringToken( _native.GetTag( attributeIndex, tagIndex ) ) );
			}
		...
		for ( int i = 0; i < _native.GetPartCount(); i++ )
		{
			...
			var boneName = _native.GetBoneCount() > 0 ? _native.GetBoneName( i ) : "";
		...
		{
			_joints.Add( new Joint( _native.GetJoint( i ) ) );
		}
	}
	...
	public int BoneCount => _native.GetBoneCount();
	...
	public sealed class BodyPart
	{
		...
		public class SpherePart : Part
		{
			...
			/// </summary>
			public void GetOutline( out Vector3[] vertices, out int[] indices )
			{
				var arrVectors = CUtlVectorVector.Create( 0, 0 );
				var arrIndices = CUtlVectorUInt32.Create( 0, 0 );
				native.GetOutline( arrVectors, arrIndices );

				vertices = new Vector3[arrVectors.Count()];
				indices = new int[arrIndices.Count()];

				for ( int i = 0; i < vertices.Length; i++ )
					vertices[i] = arrVectors.Element( i );

				for ( int i = 0; i < indices.Length; i++ )
					indices[i] = (int)arrIndices.Element( i );
		...
		public class MeshPart : Part
		{
			...
			{
				var count = mesh.GetVertexCount();
				var result = new Vector3[count];

				for ( int i = 0; i < count; i++ )
					result[i] = mesh.GetVertex( i );
			...
			public int[] GetIndices()
			{
				...
				for ( int i = 0; i < count; i++ )
				{
					mesh.GetTriangleIndices( i, out var a, out var b, out var c );
					result[i * 3 + 0] = a;
					result[i * 3 + 1] = b;
					result[i * 3 + 2] = c;
				}
